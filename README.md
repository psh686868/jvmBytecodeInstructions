...
0x10 |	bipush（byte immediate push）| 将单字节的常量值(-128~127)推送至栈顶  |
0x11 |	sipush（short immediate push）| 将一个短整型常量(-32768~32767)推送至栈顶 |
0x12 |	ldc（LoaD Constant）| 将int,float或String型常量值从常量池中推送至栈顶 |
...
0x36 |	istore | 弹出栈顶元素将栈顶int型数值存入指定本地变量     |
0x37 |	lstore | 弹出栈顶元素将栈顶long型数值存入指定本地变量    |
0x38 |	fstore | 弹出栈顶元素将栈顶float型数值存入指定本地变量   |
0x39 |	dstore | 弹出栈顶元素将栈顶double型数值存入指定本地变量  |
0x3a |	astore | 弹出栈顶元素将栈顶引用类型数值存入指定本地变量     |
